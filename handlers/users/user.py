...
@dp.message_handler(text="9", state=GradeStates.school)
@dp.message_handler(text="10", state=GradeStates.school)
@dp.message_handler(text="11", state=GradeStates.school)
...
@dp.message_handler(text="1", state=GradeStates.university)
@dp.message_handler(text="2", state=GradeStates.university)
@dp.message_handler(text="3", state=GradeStates.university)
@dp.message_handler(text="4", state=GradeStates.university)
...
@dp.message_handler(state=UserState.grade)
...
@dp.message_handler(content_types= state=TestStates.ielts)
async def ielts(message: types.Message, state: FSMContext):
    if isinstance(message.content_type, (int, float)):
        await state.update_data({"test_score": f"IELTS"})
...
async def ielts(message: types.Message, state: FSMContext):
    ...
    if isinstance(duolingo_of, (int, float)):
